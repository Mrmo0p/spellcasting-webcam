...
test('English remains unchanged and only a valid saved Thai preference selects Thai', () => {
  ...
  assert.equal(savedLocale(null), 'en');
  assert.equal(savedLocale('unexpected'), 'en');
  assert.equal(savedLocale('th'), 'th');
  assert.equal(translate('constructor', 'th'), 'constructor');
  assert.equal(translate('SPELLBOUND', 'th'), 'SPELLBOUND');
});
test('Thai covers every rune, practice tip and recognition failure', () => {
  for (const rune of RUNES) {
    for (const value of [
      ...
      rune.shape,
      rune.effect,
      DRAWING_TIPS[rune.id],
    ])
      assert.notEqual(translate(value, 'th'), value);
  }
  for (const reason of [
    'too-short',
    'too-small',
    ...
    const text = recognitionHint(reason);
    assert.notEqual(translate(text, 'th'), text);
  }
});
test('Thai translates dynamic messages while retaining numbers and spell meaning', () => {
  assert.equal(
    translate('Fireball detected — the target was Ward.', 'th'),
    'ตรวจพบลูกไฟ — รูนเป้าหมายคือเกราะเวท',
  );
  assert.equal(
    translate('Ember volley hit for 18.', 'th'),
    'ห่าลูกไฟสร้างความเสียหาย 18',
  );
  assert.match(
    translate(blockedSpellHint('ward', 'cooldown', 1250), 'th'),
    /เกราะเวท.*2 วินาที/,
  );
  assert.match(
    translate('3 of 4 prompted attempts correct (75%).', 'th'),
    /3 จาก 4 ครั้ง \(75%\)/,
  );
...
test('Thai covers the PvP lobby, match state and dynamic player events', () => {
  for (const text of [
    ...
    'INCOMING SPELL',
    'Request rematch',
    'Camera is off. The duel continues.',
  ])
    assert.notEqual(translate(text, 'th'), text);
  assert.equal(translate('Mali cast fireball.', 'th'), 'Mali ร่ายลูกไฟ');
  assert.equal(translate('Mali cast star.', 'th'), 'Mali ร่ายดาว');
  assert.notEqual(
    translate("Mali's fireball fizzled.", 'th'),
    "Mali's fireball fizzled.",
  );
  assert.notEqual(
    translate('Mali lost 16 health.', 'th'),
    'Mali lost 16 health.',
  );
  assert.notEqual(
    translate(
...
test('presentation localization preserves form values, refs and callbacks', () => {
  ...
  const translated = localizeTree(input, 'th') as ReturnType<
    typeof createElement
  >[];
  assert.ok(isValidElement(translated[0]));
  const props = translated[0].props as Record<string, unknown>;
  assert.equal(props.value, 'Practice');
  assert.equal(props.placeholder, 'อุปกรณ์ที่ใช้ทดสอบ');
  assert.equal(props.onChange, callback);
  ...
  const html = renderToStaticMarkup(
    createElement('div', null, localizeTree(button, 'th')),
  );
  assert.match(html, /aria-label="เปิดกล้อง"/);
  assert.match(html, />เปิดกล้อง<\/button>/);
