...
export function useTracking(onSample:(sample:TrackingSample)=>void){
 ...
 try{
  ...
  const stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:facing==='environment'?{exact:'environment'}:'user',width:{ideal:640},height:{ideal:480},frameRate:{ideal:30,max:30}},audio:false});
