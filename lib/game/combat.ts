...
export function tickCombat(state:CombatState,delta:number,accuracy:Accuracy):CombatState{
 ...
 const s={...state,cooldowns:{...state.cooldowns}};let left=delta;
 ...
 while(left>0&&!s.outcome){
  ...
  if(s.playerBurn){s.playerBurnTick-=step;if(s.playerBurnTick<=0){s.player=Math.max(0,s.player-BURN.damage);s.playerBurnTick=BURN.interval;s.message='You take 4 burn damage. Cast Water on yourself.';}}
  if(s.opponentBurn){s.opponentBurnTick-=step;if(s.opponentBurnTick<=0){s.opponent=Math.max(0,s.opponent-BURN.damage);s.opponentBurnTick=BURN.interval;s.message='The Archivist takes 4 burn damage.';}}
  // Burn ticks at the same timestamp as extinguishing resolve first.
  finish(s);if(s.outcome)break;
  if(waterCasting&&s.opponentWaterCast===0){s.opponentBurn=false;s.opponentBurnTick=0;s.opponentWaterCooldown=BURN.opponentWaterCooldown;s.message='The Archivist casts Water on itself. Burn extinguished.';}
...
export function castSpell(state:CombatState,command:SpellCommand):{state:CombatState;accepted:boolean;reason:string|null}{
 ...
 if(id==='water'){s.playerBurn=false;s.playerBurnTick=0;}
