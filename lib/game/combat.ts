...
export const COMBAT_VERSION='star-stun-v2';
...
export function tickCombat(state:CombatState,delta:number,accuracy:Accuracy):CombatState{
 ...
 while(left>0&&!s.outcome){
  const stunned=s.opponentStun>0;
  if(!stunned&&s.opponentBurn&&s.opponentWaterCooldown===0&&s.opponentWaterCast===0)s.opponentWaterCast=BURN.opponentWaterCast;
  const waterCasting=!stunned&&s.opponentWaterCast>0;
  const step=Math.min(left,stunned?s.opponentStun:waterCasting?s.opponentWaterCast:s.remaining,s.playerBurn?s.playerBurnTick:Infinity,s.opponentBurn?s.opponentBurnTick:Infinity,s.opponentBurn&&!waterCasting&&s.opponentWaterCooldown>0?s.opponentWaterCooldown:Infinity);
  ...
  s.opponentStun=Math.max(0,s.opponentStun-step);
  if(!stunned){if(waterCasting)s.opponentWaterCast=Math.max(0,s.opponentWaterCast-step);else s.remaining=Math.max(0,s.remaining-step);}
...
export function castSpell(state:CombatState,command:SpellCommand):{state:CombatState;accepted:boolean;reason:string|null}{
 ...
 if(id==='star')s.opponentStun=3000;
 if(id==='frost')s.remaining+=3000;
 if(id==='dispel'){s.phase='recovery';s.remaining=2200;s.blocked++;}
 s.cooldowns[id]=runeById(id).cooldown;
 s.message=id==='fireball'?'Fireball ignites the Archivist. Burn deals 4 damage each second.':id==='water'?'Water cast on yourself. Burn extinguished.':id==='star'?'Star stunned the Archivist for 3 seconds.':runeById(id).name+' cast.';
