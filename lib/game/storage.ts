...
export function loadAccuracy():Accuracy{try{const data=JSON.parse(localStorage.getItem('spellbound.accuracy.v1')||'null');if(data&&data.water===undefined)data.water={attempts:0,correct:0};if(data&&RUNE_IDS.every(id=>Number.isInteger(data[id]?.attempts)&&data[id].attempts>=0&&Number.isInteger(data[id]?.correct)&&data[id].correct>=0&&data[id].correct<=data[id].attempts))return data;}catch{}return blankAccuracy();}
...
export function loadStudy():Study|null{
 try{
  ...
  if(!s||s.version!==1||typeof s.id!=='string'||!Array.isArray(s.trials)||!Array.isArray(s.duels)||!Array.isArray(s.conditions)||s.conditions.length!==2||s.conditions.some(c=>!['fixed','adaptive'].includes(c))||!Array.isArray(s.order)||![60,70].includes(s.order.length)||!Array.isArray(s.practiceOrder)||![12,14].includes(s.practiceOrder.length)||[...s.order,...s.practiceOrder].some(r=>!RUNE_IDS.includes(r))||!['practice','measured','duel-ready','duel','rating','complete'].includes(s.status))return null;
