...
export function loadAccuracy():Accuracy{try{const data=JSON.parse(localStorage.getItem('spellbound.accuracy.v1')||'null');if(data&&RUNE_IDS.every(id=>Number.isInteger(data[id]?.attempts)&&data[id].attempts>=0&&Number.isInteger(data[id]?.correct)&&data[id].correct>=0&&data[id].correct<=data[id].attempts))return data;}catch{}return blankAccuracy();}
